...
a method. Training-free frequency manipulation is itself an active line:
\citet{si2024freeu} rebalance backbone and skip contributions inside the
U-Net at inference, and \citet{yu2025frequency} regulate wavelet sub-bands
during sampling to mitigate exposure bias. Our operation is cruder than
either, applying to finished images rather than to intermediate states.

The contribution is the \emph{use} rather than the operation. Those works
present frequency manipulation as a method and report what it gains; we use it
as a \emph{ruler}, to price what a trained objective's own stated mechanism is
worth before asking whether the objective delivers it. That reading does not
appear to be standard, and it is what turns ``the metrics did not move'' into a
quantity.
...
we have not answered. What we have answered, for the objective audited here,
is whether its own method-level extensions escape: every added axis of
adaptivity in \S\ref{sec:extensions} paid more than it returned. We would
expect the answer to depend on resolution: at
$32\times32$ the high-frequency band carries little of the signal that
Inception features encode, and the trade a frequency-aware objective makes
may look different where it carries more.
