...
On a second task---subject-driven fine-tuning of a latent diffusion model---the
same accounting is starker still: the objective corrects
\dbCorrection~\si{\decibel}, three times what it corrects on CIFAR-10, and a
post-process removing the deficit entirely changes the CLIP metrics by two
orders of magnitude less than the seed variance. The correction is real,
larger, and worth nothing those metrics can register.
