...
\subsection{Sampler sensitivity}
\label{sec:sampler-check}

Every comparison so far uses DDIM with \num{100} steps, and few-step samplers
produce smoother output --- precisely where a frequency-aware objective should
have its advantage, so the choice is not neutral with respect to the hypothesis.
Table~\ref{tab:ddpm1000} re-scores the tier-1 arms under DDPM-1000, the
published sampler, to check whether the null is an artifact of the DDIM
operating point.

\begin{table}[t]\centering
\begin{tabular}{lrr}
\toprule
config & FID $\downarrow$ & $\Delta$ vs.\ MSE \\
\midrule
MSE             & $16.54 \pm 0.16$ & --- \\
static\_wavelet & $16.85 \pm 0.09$ & $+0.31$\textsuperscript{$\dagger$} \\
AWWL            & $17.52 \pm 0.14$ & $+0.98$\textsuperscript{$\dagger$} \\
\bottomrule
\end{tabular}
\caption{Tier-1 arms under DDPM-1000 ($1000$ steps, $10\,000$ samples,
 five seeds).  \textsuperscript{$\dagger$}~$p(\text{Holm}) < 0.01$.}
\label{tab:ddpm1000}
\end{table}

Two findings matter.  First, the null survives: AWWL is still
$+0.98$~FID worse than MSE ($p(\text{Holm}) = 0.0002$), and
static\_wavelet is $+0.31$~worse ($p = 0.0022$).  Neither correction
pays for itself under the published sampler.  Second, the penalty
\emph{shrinks} for AWWL ($+1.20$ under DDIM-100, $+0.98$ under
DDPM-1000) while it \emph{grows} for static\_wavelet ($+0.20$ to
$+0.31$), consistent with the temporal schedule contributing positively
(\S\ref{sec:mechanism}) and more so when the sampler gives it more
denoising steps to operate in.  The direction is interesting; the
magnitude is not enough to close the gap.
